...
{
public:
    Sink(	float * var1, uint8_t port1=0,
			float * var2=NULL, uint8_t port2=0
			float * var3=NULL, uint8_t port3=0
			float * var4=NULL, uint8_t port4=0
			) :
    {
		if (var1) { _ports.pushback(port1); _vars.pushback(var1); }
		if (var2) { _ports.pushback(port2); _vars.pushback(var2); }
		if (var3) { _ports.pushback(port3); _vars.pushback(var3); }
		if (var4) { _ports.pushback(port4); _vars.pushback(var4); }
    }
    virtual void update(const float & dt)
    {
		for (i=1;i<_input.getSize();i++)
        	*(_var[i]) = _input[_ports[i]]; 
    }
private:
    Vector<float*> _vars;
    Vector<uint8_t> _ports;
};
...
{
public:
    Source(	float * var1, float * var2=NULL,float * var3=NULL,float * var4=NULL) :
    {
		if (var1) { _output.pushback(var1); }
		if (var2) { _output.pushback(var2); }
		if (var3) { _output.pushback(var3); }
		if (var4) { _output.pushback(var4); }
    }
};
